...
public ModelAndView edit(ModelAndView mav,@PathVariable Long id) {
	...
	//DBから一意のIDのレシピ,ステップ,材料データをもってくる
	Optional<Recipe> data = recipeService.findById((long)id);
	List<Step> steps = recipeService.findByRecipeId(id);
	List<Recipe_Ingredient> recipe_ingredients = recipeService.findByRecipe(id);
	if(data.isPresent()) {
		//とってきたデータたちをDTOに詰め替える
		RecipeDTO dto = recipeService.convertToDto(data.get(), steps,recipe_ingredients);
		mav.addObject("recipeDTO",dto);
	}else {
...
//保存
@PostMapping("/edit")
public ModelAndView update(@ModelAttribute RecipeDTO recipeRequest) {
	try {
